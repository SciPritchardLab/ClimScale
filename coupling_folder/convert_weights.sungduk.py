    ...
    with h5py.File(weights_file_name,'r') as weights_file:
        ...
        for idx,layer in enumerate(layer_config):
            ...
            if class_name not in SUPPORTED_LAYERS:
                ...
                # set name 2 (sungduk)
                # 'batchnormalization' would need a similar modification in future
                if len(model_weights[name].keys()) == 1:
                    name2  = model_weights[name].keys().__str__().split("'")[1]
                else:
                    print('Error: there are multiple keys under model_weights[dense]')
                    exit()

                # get weights and biases out of dictionary
                layer_weights = np.array(
                    model_weights[name][name2]['kernel:0']
                )

                if 'bias:0' in model_weights[name][name2]:
                    layer_bias = np.array(
                        model_weights[name][name2]['bias:0']
                    )
                else:
